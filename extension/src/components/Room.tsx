...
interface StandaloneProblem {
    title: string;
    titleSlug: string;
}

let copyIconTimer: ReturnType<typeof setTimeout>;
...
export default function Room({
    ...
    function readStandaloneProblem() {
        let standaloneProblemString = localStorage.getItem(
            "tipsBoardStandaloneProblem"
        );
        if (!standaloneProblemString) {
            return;
        }

        try {
            let parsedStandaloneProblem: StandaloneProblem = JSON.parse(
                standaloneProblemString
            );
            if (
                questions.some(
                    (question) =>
                        question.titleSlug === parsedStandaloneProblem.titleSlug
                )
            ) {
                syncAcceptedSubmission(parsedStandaloneProblem);
            }
        } catch {
            localStorage.removeItem("tipsBoardStandaloneProblem");
        }
    }

    async function syncAcceptedSubmission(problem: StandaloneProblem) {
        ...
        if (!socket.connected) {
            socket.once("connect", () => {
                syncAcceptedSubmission(problem);
            });
            return;
        }

        let submissionUrl = `https://leetcode.com/problems/${problem.titleSlug}/submissions/standalone/`;
        ...
        let submissionRequestBody: SubmissionRequestBody = {
            ...
            questionTitleSlug: problem.titleSlug,
            url: submissionUrl,
        };
    ...
    useEffect(() => {
        ...
        let storedRoomMessagesString =
            localStorage.getItem("tipsBoardMessages");
        ...
        async function handleClickSubmitCodeButton(event: MessageEvent) {
            if (
                ...
                !["submit", "accepted", "verdict"].includes(
                    event.data?.event
                ) ||
            ...
            window.parent.postMessage(
                {
                    ...
                    submissionEventId: event.data.submissionEventId,
                },
                event.origin
            );

            if (
                processedSubmissionEventIds.current.has(
                    event.data.submissionEventId
                )
            ) {
                return;
            }
            processedSubmissionEventIds.current.add(
                event.data.submissionEventId
            );
    ...
    useEffect(() => {
        readStandaloneProblem();
        window.addEventListener("focus", readStandaloneProblem);

        return () => {
            window.removeEventListener("focus", readStandaloneProblem);
        };
    }, [questions]);
